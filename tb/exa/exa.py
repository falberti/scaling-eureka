...
######################################################################
try:
    lxd_client.images.get_by_alias(NAME+"-frr")
    print("Image {} already exists. Exiting...".format(NAME+"-frr"))
    sys.exit(1)
except pylxd.exceptions.NotFound as e:
    pass
...
try:
    ...
    print(e)
    sys.exit(1)
print("Done with creating image {}".format(NAME+"-frr"))

try:
    lxd_client.images.get_by_alias(NAME+"-exa")
    print("Image {} already exists. Exiting...".format(NAME+"-exa"))
    sys.exit(1)
except pylxd.exceptions.NotFound as e:
    pass
...
try:
    ...
    print(e)
    sys.exit(1)
print("Done with creating image {}".format(NAME+"-exa"))
...
config = {'name': container_name,
          'source': {'type': 'image', 'alias': NAME+'-exa'},
          'public': False,
          'auto_update': False,
          'profiles': ['default', NAME] }
cont = lxd_client.containers.create(config, wait=True)
...
try:
    container = lxd_client.containers.get(NAME+'-exa')
    # Change permissions of exa files
    container.execute(['chmod', '-R', '777', '/root'])
    out = container.execute(['/usr/bin/screen', '-d', '-m', '/usr/sbin/exabgp', '/root/exabgp.conf'])
except pylxd.exceptions.NotFound as e:
